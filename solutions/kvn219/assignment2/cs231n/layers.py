...
def affine_backward(dout, cache):
    ...
    dw = np.dot(x.reshape(dims).T, dout).reshape(w.shape)
...
def batchnorm_backward(dout, cache):
    ...
    """
    # shamelessly taken from https://github.com/cthorey/CS231/blob/master/assignment2/cs231n/layers.py
...
def conv_backward_naive(dout, cache):
    ...
    for i, j in itertools.product(range(Hh), range(Hw)):
        ...
        selected_window = x_padding[:, :, start_h:end_h, start_w:end_w]
        for k in xrange(F):
            dw[k] += np.sum(selected_window * (dout[:, k, i, j])[:, None, None, None], axis=0)
...
def max_pool_forward_naive(x, pool_param):
    ...
    Hh = (H - HH) / stride + 1
    Hw = (W - WW) / stride + 1
    out = np.zeros((N, C, Hh, Hw))

    for i, j in itertools.product(range(Hh), range(Hw)):
        start_h, end_h = i * stride, i * stride + HH
        start_w, end_w = j * stride, j * stride + WW
        selected_window = x[:, :, start_h:end_h, start_w:end_w]
        out[:, :, i, j] = np.max(selected_window, axis=(2, 3))
    #############################################################################
    # END OF YOUR CODE                              #
    #############################################################################
    cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
    ...
    N, C, H, W = x.shape
    HH, WW, stride = pool_param['pool_height'], pool_param['pool_width'], pool_param['stride']
    Hh, Hw = ((H - HH) / stride + 1, (W - WW) / stride + 1)
    dx = np.zeros_like(x)
    for i, j in itertools.product(range(Hh), range(Hw)):
        start_h, end_h = i * stride, i * stride + HH
        start_w, end_w = j * stride, j * stride + WW
        ...
        temp_binary = val[:, :, None, None] == selected_window
        dx[:, :, start_h:end_h, start_w:end_w] +=\
            temp_binary * (dout[:, :, i, j])[:, :, None, None]
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    return dx
